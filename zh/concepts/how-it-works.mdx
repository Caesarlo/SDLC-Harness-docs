...
Solo 模式下，`verify` 按需创建临时 `claim`，单人用户通常感觉不到租约。Team 模式要求显式 `claim`；过期后才能被其他 `owner` 接管。两种模式都会拒绝与他人的活跃 `claim` 竞争写入。
...
- review 绑定当前 commit，评审者不能是该功能 `claim` 的 `owner`，并且必须晚于最新验证。
