...
<CardGroup cols={2}>
  <Card title="10 分钟接入" icon="rocket" href="/zh/quickstart">
    在已有仓库或空仓库中生成协议，启用 Git hook，并完成第一个功能。
  </Card>
  <Card title="为什么需要它" icon="circle-question" href="/zh/concepts/why-sdlc-harness">
    了解编码 Agent 跨会话和并行协作时常见的失效模式。
  </Card>
  <Card title="完整日常流程" icon="list-check" href="/zh/guides/daily-workflow">
    从选择任务到验证、评审、完成和会话收尾。
  </Card>
  <Card title="命令参考" icon="terminal" href="/zh/reference/commands">
    查询全部命令、参数、前置条件和状态变化。
  </Card>
</CardGroup>
...
- `feature_list.json` 保存里程碑、功能、依赖、`status`、`claim` 和证据。
- [`sdlc-harness verify`](/zh/reference/command-evidence#verify) 真实执行功能声明的验证命令。
- [`sdlc-harness review record`](/zh/reference/command-evidence#review-record) 记录独立评审结论。
- [`sdlc-harness feature complete`](/zh/reference/command-feature#complete) 原子检查完成条件并更新状态。
- [`sdlc-harness validate`](/zh/reference/command-repository#validate) 审核整个仓库，并用非零退出码阻止不合规提交。
...
SDLC Harness 是协议和治理层，不是测试框架、编码 Agent、项目管理系统或需求正确性证明工具。它让过程可追踪、证据可核验、规则可执行；实际产品判断、测试内容和部署命令仍由你的项目负责。

<Card title="开始接入" icon="arrow-right" href="/zh/quickstart">
