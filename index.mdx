...
- [`sdlc-harness verify`](/reference/command-evidence#verify) 真实执行功能声明的验证命令。
- [`sdlc-harness review record`](/reference/command-evidence#review-record) 记录独立评审结论。
- [`sdlc-harness feature complete`](/reference/command-feature#complete) 原子检查完成条件并更新状态。
- [`sdlc-harness validate`](/reference/command-repository#validate) 审核整个仓库，并用非零退出码阻止不合规提交。
